...
const testimonials = [
  {
    ...
    rating: 5,
    comment: 'Green Label has transformed our medical waste handling. Their team is professional, punctual, and fully compliant with NEMA regulations. We have not had a single incident since partnering with them.',
    recommend: true,
  },
  {
    name: 'James Okello',
    ...
    rating: 5,
    comment: 'Outstanding service in managing our drilling waste and hazardous materials. Green Label understands the strict requirements of the oil and gas sector and delivers consistently.',
    recommend: true,
  },
  {
    name: 'Grace Akello',
    ...
    rating: 5,
    comment: 'Reliable and affordable waste collection for all our retail outlets across Kampala. Their recycling programme has helped us reduce waste costs by 30%.',
    recommend: true,
  },
  {
    name: 'Peter Musoke',
    ...
    rating: 5,
    comment: 'We needed a partner who could handle large-scale industrial waste safely. Green Label exceeded expectations — their equipment is modern and their documentation is impeccable.',
    recommend: true,
  },
  {
    name: 'Esther Birungi',
    ...
    rating: 4,
    comment: 'Green Label has been instrumental in keeping Mbarara clean. Their community engagement and education programmes make them much more than just a waste collector.',
    recommend: true,
  },
  {
    ...
    rating: 5,
    comment: 'The level of professionalism and regulatory compliance is world-class. Green Label is the only waste management company I trust with hazardous materials in the Albertine region.',
    recommend: true,
  },
]
    ...
    <>
        ...
        <div className="relative z-10 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <ScrollRevealSection>
            <h2 className="reveal reveal-up mb-2 text-center font-heading text-2xl font-bold text-gray-900 md:text-3xl">
              <i className="fa-solid fa-chart-bar mr-2 text-brand-green" aria-hidden="true" />Ratings by Service
            </h2>
          ...
          <CardGrid columns={3}>
            {serviceHighlights.map((s) => (
              <div key={s.title} className="card-premium rounded-2xl border-b-2 border-b-brand-orange bg-white p-4 text-center shadow-md">
      ...
      <section className="relative overflow-hidden bg-gradient-warm py-12 md:py-16">
        ...
        <div className="relative z-10 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <ScrollRevealSection>
            <h2 className="reveal reveal-up mb-2 text-center font-heading text-2xl font-bold text-gray-900 md:text-3xl">
              <i className="fa-solid fa-quote-left mr-2 text-brand-green" aria-hidden="true" />Client Testimonials
            </h2>
            <p className="reveal reveal-up stagger-1 mx-auto mb-8 max-w-xl text-center text-sm text-gray-600">
          ...
          <ScrollRevealSection>
            <div className="grid grid-cols-2 gap-3 md:gap-6 lg:grid-cols-3">
              {testimonials.map((t, i) => (
                <div key={t.name} className={`reveal reveal-up stagger-${Math.min(i + 1, 6)} card-premium relative overflow-hidden rounded-2xl border border-gray-100 bg-white p-4 md:p-6 shadow-md`}>
                  <i className="fa-solid fa-quote-left absolute -right-2 -top-2 text-6xl text-brand-green/5" aria-hidden="true" />
                  <div className="relative z-10">
                    <div className="mb-3 flex items-center justify-between">
                      <div className="flex gap-0.5">
                        {Array.from({ length: t.rating }, (_, j) => (
                          <i key={j} className="fa-solid fa-star text-xs text-amber-400" aria-hidden="true" />
                        ))}
                      </div>
                      {t.recommend && (
                        <span className="rounded-full bg-green-100 px-2 py-0.5 text-[10px] font-medium text-green-700">
                          <i className="fa-solid fa-thumbs-up mr-1" aria-hidden="true" />Recommends
                        </span>
                      )}
                    </div>
                    <p className="mb-3 text-xs leading-relaxed text-gray-600">&ldquo;{t.comment}&rdquo;</p>
                    <div className="flex items-center gap-2 border-t border-gray-100 pt-3">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-gray-500">{t.role}, {t.org}</p>
                      </div>
                    </div>
                    <span className="mt-2 inline-block rounded-full bg-brand-green/10 px-2 py-0.5 text-[10px] font-medium text-brand-green">{t.service}</span>
                  </div>
                </div>
              ))}
            </div>
      ...
      <section className="bg-gradient-subtle py-12 md:py-16">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <ScrollRevealSection>
            <h2 className="reveal reveal-up mb-2 text-center font-heading text-2xl font-bold text-gray-900 md:text-3xl">
              <i className="fa-solid fa-shield-halved mr-2 text-brand-green" aria-hidden="true" />Why Clients Trust Us
            </h2>
            <p className="reveal reveal-up stagger-1 mx-auto mb-8 max-w-xl text-center text-sm text-gray-600">
          ...
          <CardGrid columns={3}>
            {[
              { icon: 'fa-solid fa-certificate', title: 'Fully Licensed', desc: 'NEMA-licensed for hazardous and non-hazardous waste across all regions of Uganda.' },
              { icon: 'fa-solid fa-clock', title: 'Always On Time', desc: '99.5% on-time collection rate — we never leave you waiting.' },
              { icon: 'fa-solid fa-leaf', title: 'Eco-Friendly', desc: 'Sustainable disposal methods with a focus on recycling and resource recovery.' },
              { icon: 'fa-solid fa-headset', title: '24/7 Support', desc: 'Round-the-clock emergency response and dedicated account management.' },
              { icon: 'fa-solid fa-truck', title: 'Modern Fleet', desc: 'GPS-tracked, specialised vehicles for every waste type and volume.' },
              { icon: 'fa-solid fa-file-shield', title: 'Full Compliance', desc: 'Complete documentation, manifests, and audit trails for regulatory peace of mind.' },
            ].map((item) => (
              <div key={item.title} className="card-premium flex items-start gap-3 rounded-2xl border-l-4 border-l-brand-green bg-white p-4 shadow-md">
      ...
      <section className="relative overflow-hidden bg-gradient-green py-12">
        ...
        <div className="relative z-10 mx-auto max-w-4xl px-4 text-center">
          <h2 className="font-heading text-xl font-bold text-white md:text-2xl">
            <i className="fa-solid fa-star mr-2" aria-hidden="true" />Join Our Satisfied Clients
          </h2>
