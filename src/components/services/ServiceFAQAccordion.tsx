...
export default function ServiceFAQAccordion({ faqs }: { faqs: FAQ[] }) {
  ...
  return (
    <div className="divide-y divide-gray-200 rounded-lg border border-gray-200">
      {faqs.map((faq, i) => (
        <div key={i} className={openIndex === i ? 'border-l-4 border-l-brand-green' : ''}>
          <button
          ...
          >
            ...
            <i
              className={`fa-solid ${openIndex === i ? 'fa-minus' : 'fa-plus'} shrink-0 text-brand-green transition-transform`}
            />
          </button>
          <div
